...
def model_from_tf(input_shape=(128, 128, 3), num_classes=5, is_transfer_learning=True):
    inputs = Input(shape=input_shape)
    conv_model = VGG16(weights='imagenet', include_top=False, input_tensor=inputs)
    conv_model.trainable = not is_transfer_learning
    outputs = Flatten()(conv_model.output)
